...
- **テキスト本編の段階ヒント**: 練習問題の単一「ヒント」に加え、講義テキストの**理解チェック**（`_Doc/0-基本ルール.md`「テキスト本編・理解チェックブロック」）では、summary を **`ヒント①` / `ヒント②` / `ヒント③`** のように**複数の details** で並べてよい。いずれもヒント系の色（`#555`）を用い、**解答例**は従来どおり 1 ブロック（テーマ色）にまとめる。
