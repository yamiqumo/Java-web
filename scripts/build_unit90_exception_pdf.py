"""
Unit90 例外処理 講義HTMLから印刷用HTMLを生成し、Chrome headless で PDF を出力する。
...
def main() -> int:
	...
	src = root / "Unit90_Exception" / "01-テキスト-Java例外処理.html"
	if not src.exists():
	...
	pdf_html = root / "Unit90_Exception" / "src" / "template.html"
	pdf_html.parent.mkdir(parents=True, exist_ok=True)
	doc = f"""<!DOCTYPE html>
<html lang="ja">
<head>
<meta charset="utf-8">
...
"""
	...
	if chrome is None:
		print(
			"CHROME_NOT_FOUND: PDFは手動で「Unit90_Exception/src/template.html」を開き印刷してください。",
			file=sys.stderr,
		)
		return 0

	out_pdf = root / "Unit90_Exception" / "99-テキストpdf-Java例外処理.pdf"
