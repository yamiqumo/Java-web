"""
Unit04 DAO（更新系）講義HTMLから印刷用HTMLを生成し、Chrome headless で PDF を出力する。
...
def main() -> int:
	...
	src = root / "Unit04_DAO_Update" / "01-テキスト-DAO更新とトランザクション.html"
	if not src.exists():
	...
	pdf_html = root / "Unit04_DAO_Update" / "src" / "template.html"
	doc = f"""<!DOCTYPE html>
<html lang="ja">
<head>
<meta charset="utf-8">
...
"""
	...
	if chrome is None:
		print("CHROME_NOT_FOUND: PDFは手動で「Unit04_DAO_Update/src/template.html」を開き印刷してください。", file=sys.stderr)
		return 0

	out_pdf = root / "Unit04_DAO_Update" / "99-テキストpdf-DAO更新とトランザクション.pdf"
